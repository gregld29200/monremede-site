...
import type { PostExpanded } from '@/types/admin'
...
export default function EditBlogPage({ params }: PageProps) {
  const { id } = use(params)
  const [post, setPost] = useState<PostExpanded | null>(null)
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await fetch(`${API_ADMIN_PATH}/blog/${id}`)
        if (!response.ok) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <h3 className="font-display text-lg text-forest mb-1">{error || 'Article non trouvé'}</h3>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="font-display text-2xl text-forest">Modifier l&apos;article</h2>
